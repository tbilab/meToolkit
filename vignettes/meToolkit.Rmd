...
To do this, the data must be loaded into a path (relative to the main app working directory) provided to `run_data_loader()` in the `preloaded_data_path` argument. 

__Phenome Data:__ If the patient phenome file is common between all preloaded results it can be stored at `preloaded_data_loc/id_to_code.csv`. If it is unique to a given result set it can be set in that SNP's subfolder with the same name: e.g. `preloaded_data_loc/<SNP_ID>/id_to_code.csv`. If a phenome mapping dataset is provided for a SNP it will always take priority over the app-wide mapping. 

__Genome Data:__ Each individual SNP's patient-to-snp data us stored in `preloaded_data_loc/<SNP_ID>/id_to_snp.csv`

__PheWAS Results:__ The PheWAS results for the SNP go in as `preloaded_data_loc/<SNP_ID>/phewas_results.csv` respectively. 


As an example, this is what the file preloaded directory structure looks like for a hosted PheWAS-ME application with two preloaded snps. 

```
$ tree app_w_preloaded_data/
├── app.R
└── preloaded_data
    ├── id_to_code.csv
    ├── rs123456
