...
embed_png <- function(image_title, dpi = NULL, width = "100%") {
  
  base_path <- system.file("figures/", package = "meToolkit")
  full_path <- paste0(base_path, image_title)
  meta <- attr(png::readPNG(full_path, native = TRUE, info = TRUE), "info")
  if (!is.null(dpi)) meta$dpi <- rep(dpi, 2)
  knitr::asis_output(
    glue::glue(
      "
      <div style=\"text-align: center;\">
      <img src=\"{full_path}\" width={width} />
      </div>
      "
    )
  )
...
At anytime a csv of all the subjects that have one or more of the currently selected phenotypes can be downloaded using the "Selected subjects" button at the top right of the app. 
