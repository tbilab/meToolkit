#' UI function of pattern_network module
#'
#' @param id String with unique id of module in app
#' @param div_class A character string containing a class name for the entire plot to be wrapped in. This can then be used to style with external css. Defaults to 'pattern_network_plot'.
#' @return HTML component of shiny module
#' @export
#'
#' @examples
#' pattern_network_UI('my_mod')
pattern_network_UI <- function(id, div_class = 'pattern_network_plot') {
  ns <- NS(id)
  tagList(
    div(
      div(class = div_class,
...
#' Server function of pattern_network module
#'
...
#'
#' @examples
#' callModule(pattern_network, 'my_mod')
pattern_network <- function(input, output, session, individual_data, all_patient_snps, action_object = NULL) {
  ...
  output$chart <- r2d3::renderD3({
    ...
    # Send everything to the pattern_network javascript code
    r2d3::r2d3(
      data = list(
        data = pattern_to_enrichment,
        options = list(marginalData = code_marginal_data)
      ),
      script = system.file("d3/pattern_network/main.js", package = "meToolkit"),
      dependencies = c("d3-jetpack",system.file("d3/pattern_network/helpers.js", package = "meToolkit"))
    )
  }) # End renderD3
