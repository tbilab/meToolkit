...
#' dataframe.
#'
#' @param phenome_data Dataframe with columns \code{id} and \code{code}
#'   corresponding to all pairs of case and phenotypes
#' @param genome_data Dataframe with columns \code{id} and \code{snp}
#'   corresponding to all pairs of case and snp MA status for a given individual
#'   snp
#'
#' @return A wide dataframe with columns \code{id}, \code{snp}, and all the
#'   unique phecodes.
#' @export
#'
#' @examples
...
mergePhenomeGenome <- function(phenome_data, genome_data){
  phenome_data %>%
    ...
    dplyr::left_join(genome_data, by = 'id') %>%
